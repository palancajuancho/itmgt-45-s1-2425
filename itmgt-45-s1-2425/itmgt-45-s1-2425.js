...
function savings(grossPay, taxRate, expenses) {
    // Step 1: Calculate after-tax pay and round it down
    let afterTaxPay = Math.floor(grossPay * (1 - taxRate));

    // Step 2: Subtract expenses from after-tax pay
    let remainingPay = afterTaxPay - expenses;
...
function materialWaste(totalMaterial, materialUnits, numJobs, jobConsumption) {
    // Step 1: Calculate total material consumed
    let totalConsumed = numJobs * jobConsumption;

    // Step 2: Subtract total material consumed from available material
    let remainingMaterial = totalMaterial - totalConsumed;

    // Step 3: Format the output as a string with the material unit
 ...
 *
 * To get simple interest, multiply the principal to the quantity (rate * time). Add this amount to the principal to get the final value.
 *
